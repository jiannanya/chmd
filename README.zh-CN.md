...
微基准覆盖 22 个场景，默认每组重复 10 次，可用 `chmd_benchmark 20` 修改轮数。CSV 覆盖混合文本、普通段落、空行、表格、引用、HTML 注释、Unicode 文本以及多种不匹配定界符。解析、HTML、紧凑 AST 和事件输出分别计时，同时输出节点数、保留堆字节、峰值堆字节和分配次数。堆指标为基准程序通过普通 `new`/`delete` 记录的申请量，排除调用方输入、分配器元数据和线程栈；它不是进程 RSS。
